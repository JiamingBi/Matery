...
namespace mpp { namespace utils { namespace profile {
    ...
    void ProfilerTimer::Stop()
    {
        ...
        int64_t start = std::chrono::time_point_cast<std::chrono::microseconds>(GetStartTimepoint())
                            .time_since_epoch()
                            .count();
        int64_t end = std::chrono::time_point_cast<std::chrono::microseconds>(GetEndTimepoint())
                          .time_since_epoch()
                          .count();
